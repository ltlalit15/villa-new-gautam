...
export const signUp = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            `INSERT INTO users 
            (name, email, password, phone_number, assigned_villa_id, status, image)
            VALUES (?, ?, ?, ?, ?, ?, ? )`,
            [
                name,
                email,
...
export const createVilla = async (req, res) => {
  try {
    ...
    const sql = `
      INSERT INTO villas 
      (villa_name, location, type, price, area, status, description, image, owner_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    res.json({ message: "Villa created successfully" });
  } catch (error) {
...
//   try {
//     const [rows] = await pool.query("SELECT * FROM villas ORDER BY id DESC");
//     res.json(rows);
//   } catch (error) {
...
export const getVillaById = async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM villas WHERE id = ?", [
      req.params.id,
    ]);
    res.json(rows[0]);
  } catch (error) {
...
export const updateVilla = async (req, res) => {
  try {
    ...
    let imageUrl = req.body.existingImage; // keep old image if not updating
    ...
    const sql = `
      UPDATE villas 
      SET villa_name=?, location=?, type=?, price=?, area=?, status=?, description=?, image=?,owner_id=?
      WHERE id=?
    `;
    ...
    res.status(500).json({ error: "Error updating villa" });
  }
};
...
export const deleteVilla = async (req, res) => {
  try {
    await pool.query("DELETE FROM villas WHERE id = ?", [req.params.id]);
...
export const getVillasByOwnerId = async (req, res) => {
    try {
        ...
        if (villas.length === 0) {
            return res.status(404).json({ message: "No villas found for this owner" });
        }
        ...
        console.error("Get Villas by Owner Error:", error);
...
export const createBooking = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            ...
            (guest_name, email, phone, villa_id, check_in, check_out, price, payment_method, status, payment_status, notes, owner_id)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                guest_name,
                email,
    ...
    try {
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                assigned_villa_id: user.assigned_villa_id,
                status: user.status,
                image: user.image
            }
        });
